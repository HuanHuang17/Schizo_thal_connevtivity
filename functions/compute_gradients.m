function [Gx_org,Gy_org,Gz_org]=compute_gradients(img_pca,ins_msk,Subject,Figures,Streamlines, Mag, CCMaskFile)
...
% Mag=0:print out eigenmap slices; 
...
if ~isempty(Subject)
    if Mag == 1
        ...
        mat2nii(img_mag,[Subject,'magnitude91x109x91.nii'], [91 109 91], 32, CCMaskFile);
    elseif Mag == 0
        fprintf('Write out %s\n',[Subject,'eigenvector.nii'])
        mat2nii(img_pca(min_x:max_x,min_y:max_y,min_z:max_z),[Subject,'eigenvector.nii']);
        mat2nii(img_pca,[Subject,'eigenvector91x109x91.nii'], [91 109 91], 32, CCMaskFile);
    end
end
...
%     Mag=0; % Mag=0:print out eigenmap slices; 
    ...
    mycolormap=dlmread('/home/hujian/HH/FG/1_linux_FG/MapGradient/trackvis_jet.txt');
...
%     %Bin 256->257 is last color    
    ...
    saveas(gcf, [Subject, 'Sagittal_', imgName], 'png');
    close;
    ...
    saveas(gcf, [Subject, 'Coronal_', imgName], 'png');
    close;
    ...
    saveas(gcf, [Subject, 'Axial_', imgName], 'png');
    close;
end
